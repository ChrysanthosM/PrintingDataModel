...
import org.masouras.model.mssql.schema.qb.table.LetterSetUpTable;
import org.masouras.model.mssql.schema.qb.table.PrintingSetUpTable;
...
public class PrintingOptionsJ2SQL extends AbstractJ2<PrintingOptionsRepo.NameOfSQL> implements PrintingOptionsRepo {
    ...
    public void loadListPrintingSetUp() {
        addLoader(NameOfSQL.LIST_PRINTING_SETUP, J2SQL.create(getDataSourceType())
                ...
                .fullJoin(letterSetUpTable.as(J1)).on(t0(printingSetUpTable.LETTER_TYPE).eq(j1(letterSetUpTable.LETTER_TYPE))).fromJoinSelectOnly(j1(letterSetUpTable.XSL_TYPE, letterSetUpTable.RENDERER_TYPE, letterSetUpTable.VALID_FLAG))
                .where(j1(letterSetUpTable.VALID_FLAG).eq(ValidFlag.ENABLED.getCode()))
